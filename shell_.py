...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into shell_ (title,links,stars,sh_desc) values (%s,%s,%s,%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
    except :
...
if __name__ == "__main__":
    big_list = []
    url = 'https://github.com/search?l=Shell&o=desc&p=1&q=shell&s=stars&type=Repositories'
